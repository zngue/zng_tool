...
func NewRepo(url string, branch string) *Repo {
	return &Repo{
		url:    url,
		home:   zngHomeWithDir("repo/" + repoDir(url)),
		branch: branch,
	}
}
...
func (r *Repo) Clone(ctx context.Context) error {
	var pathInfo = r.Path()
	if _, err := os.Stat(pathInfo); !os.IsNotExist(err) {
		return r.Pull(ctx)
	}
	var cmd *exec.Cmd
	if r.branch == "" {
...
// RemoTo()
func (r *Repo) RemoTo(ctx context.Context, to string, ignores, replaces []string) (err error) {
	err = r.Clone(ctx)
	if err != nil {
		return
	}
	s := r.Path()
	err = copyDir(s, to, replaces, ignores)
	if err != nil {
		return
	}
	err = r.renameDir(to, "zng_layout", to)
	if err != nil {
		return
	}
	return
...
// 修改文件夹名称
func (r *Repo) renameDir(src, oldName, newName string) error {
	return filepath.Walk(src, func(path string, info os.FileInfo, err error) error {
		if info != nil && info.IsDir() && info.Name() == oldName {
			//修改文件夹名称
			return os.Rename(path, strings.ReplaceAll(path, oldName, newName))
		}
		return nil
	})
}
