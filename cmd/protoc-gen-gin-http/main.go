...
func main() {
	...
	opts.Run(func(gen *protogen.Plugin) error {
		...
		for _, file := range files {
			...
			g.P("\"github.com/zngue/zng_app/pkg/router\"")
			g.P("\"github.com/zngue/zng_app/pkg/validate\"")
			g.P(")")
			for serverIndex, service := range file.Services {
				g.P(routerTemplateReplace(string(service.Desc.Name())))
				var methodItems []*MethodInfo
				for _, method := range service.Methods {
					...
					methodItems = append(methodItems, &MethodInfo{
						Method: method.GoName,
						Path: func(rule *annotations.HttpRule) string {
							if rule != nil {
								if rule.GetPost() != "" {
									return rule.GetPost()
								}
						...
						IsPost: func(rule *annotations.HttpRule) bool {
							if rule != nil && rule.GetPost() != "" {
								return true
							}
							return false
						}(httpRule),
						ServerIndex: serverIndex,
						ServerName:  string(service.Desc.Name()),
					})
				}
				g.P(routerFnTemplateReplace(string(service.Desc.Name()), methodItems))
				g.P(registerTemplateReplace(string(service.Desc.Name())))
			}
			for serverIndex, service := range file.Services {
				g.P(fmt.Sprintf("type %sGinHttpService interface {", service.Desc.Name()))
				for _, method := range service.Methods {
					g.P(fmt.Sprintf(
						"%s(ctx *gin.Context,req *%s)(rs *%s,err error)",
						method.GoName,
				...
				g.P("}")
				var serviceName = fmt.Sprintf("%s", service.Desc.Name())
				for _, method := range service.Methods {
					var isPost bool
					httpOption := method.Desc.Options()
					extension := proto.GetExtension(httpOption, annotations.E_Http)
					httpRule := extension.(*annotations.HttpRule)
					if httpRule != nil && httpRule.GetPost() != "" {
						isPost = true
					}
					fnTmp := fnTemplatesReplace(
						isPost,
						serverIndex,
						serviceName,
						method.GoName,
